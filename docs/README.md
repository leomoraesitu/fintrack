# Documentacao do FinTrack

Esta pasta centraliza a documentacao de continuidade do projeto.

## Como usar esta documentacao

Use este indice como ponto de entrada para navegar entre produto, engenharia, design, decisoes tecnicas e evolucao futura.

Fluxo recomendado:

1. entender o produto e o escopo do MVP
2. revisar arquitetura, estrutura e workflow
3. consultar design e especificacoes de tela antes da implementacao de UI
4. usar ADRs e features como referencia de decisao e comportamento

## Estrutura

### Produto

- [Visao do produto](./product/product-vision.md)
- [Escopo do MVP](./product/mvp-scope.md)
- [Visao geral do backlog](./product/backlog-overview.md)

### Engenharia

- [Arquitetura](./engineering/architecture.md)
- [Estrutura do projeto](./engineering/project-structure.md)
- [Padroes de codigo](./engineering/coding-standards.md)
- [Gerenciamento de estado](./engineering/state-management.md)
- [Estrategia de testes](./engineering/testing-strategy.md)
- [Fluxo de desenvolvimento](./engineering/development-workflow.md)
- [Workflow do Codex](./engineering/codex-workflow.md)
- [Setup do Firebase](./engineering/firebase-setup.md)

### Decisoes tecnicas

- [ADR-001 - Adocao do BLoC](./adr/adr-001-adocao-bloc.md)
- [ADR-002 - Autenticacao mock](./adr/adr-002-autenticacao-mock.md)
- [ADR-003 - Persistencia local](./adr/adr-003-persistencia-local.md)
- [ADR-004 - Navegacao principal](./adr/adr-004-navegacao.md)
- [ADR-006 - Adocao do Firebase](./adr/adr-006-adocao-firebase.md)

### Features

- [Autenticacao mock](./features/auth-mock.md)
- [Transacoes](./features/transactions.md)
- [Dashboard](./features/dashboard.md)
- [Categorias e filtros](./features/categories-and-filters.md)

### Design

- [Visao geral de design](./design/README.md)
- [Handoff visual do prototipo](./design/ff-designer-handoff.md)
- [Design system](./design/design-system.md)
- [Especificacao de telas](./design/screen-specs.md)
- [Inventario de assets](./design/assets-inventory.md)

### Evolucao futura

- [Backend e sincronizacao](./future/backend-and-sync.md)
- [Release e publicacao](./future/release-and-publishing.md)
- [Contribuicao externa](./future/contributing.md)

## Ordem sugerida de leitura

1. Visao do produto
2. Escopo do MVP
3. Arquitetura
4. Estrutura do projeto
5. Gerenciamento de estado
6. Fluxo de desenvolvimento
7. Visao geral de design
8. Visao geral do backlog
9. Features
10. ADRs
11. Documentos futuros
